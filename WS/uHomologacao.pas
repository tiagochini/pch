...
constructor THomologacao.Create(aConnectionDB: TZConnection;
  fluviometro, pluviometro, Usuario, Senha: String);
begin
...
destructor THomologacao.Destroy;
begin
...
function THomologacao.CarregarMedicoesPendentes: Array_Of_medicaoTO;
var
...
function THomologacao.CriarEstacaoComMedicoes(const CodigoFlu,
  CodigoPlu: string; const Medicoes: Array_Of_medicaoTO): estacaoTO;
var
  Estacao: estacaoTO;
begin
...
function THomologacao.EnviarMedicoesSOAP(const Usuario, Senha: string;
  const Estacoes: Array_Of_estacaoTO): retornoService;
var
...
procedure THomologacao.MarcarMedicoesComoEnviadasPorDataMedicao
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        dataMedicao := StrToDate(medicoesInseridas[i].medicao[j].dataMedicao);
...
procedure THomologacao.ProcessarEnvioMedicoes(showMessage: Boolean = False);
var
...
procedure THomologacao.RegistrarLogEnvio(const Tipo, Mensagem: string;
  const Medicoes: Array_Of_medicaoTO);
var
  JsonTexto: string;
  JSONArr: TJSONArray;
